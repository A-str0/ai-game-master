...
impl TryFrom<GameSessionRow> for GameSession {
    ...
    fn try_from(row: GameSessionRow) -> RepoResult<Self> {
        let retrivial_k = u8::try_from(row.retrivial_k).map_err(|_| RepoError::Unavailable)?;
        let memory_budget = u32::try_from(row.memory_budget).map_err(|_| RepoError::Unavailable)?;
...
impl TryFrom<&GameSession> for NewGameSessionRow {
    ...
    fn try_from(session: &GameSession) -> RepoResult<Self> {
        Ok(Self {
            id: session.id().0,
            owner_id: session.owner_id().0,
            retrivial_k: i16::from(session.config().retrivial_k()),
            memory_budget: i32::try_from(session.config().memory_budget())
                .map_err(|_| RepoError::Unavailable)?,
